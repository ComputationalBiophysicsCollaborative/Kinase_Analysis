...
with open(args.stkfile) as f:
    dat = [l.split() for l in f.readlines()[:-1] if (not l.isspace()) and (not l.startswith('#'))]
    for x in dat:
        if x[0] in pdbfns:
            pdbseqs[x[0]] += x[1]
        else:
...
if args.unalign:
    ...
    unalign = concatenate([range(int(p[0]),int(p[1])+1) if len(p) > 1 else [int(p)] for p in parts])
