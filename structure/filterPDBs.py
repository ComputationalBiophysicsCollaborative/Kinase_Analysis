...
#filter out sequences with many gaps
for k in fullalnseqs.keys():
    if fullalnseqs[k].count('-') > 32:
        del fullalnseqs[k]

L = 192 #XXX generalize
...
def getDistances(struct):
    ...
    B3Katoms = c[c.resID == aln2structMap[21]]
    aCEatoms = c[c.resID == aln2structMap[33]]
    ...
    actDists = []
    for a,b in [(110,127), (110, 130), (111, 133)]:
        try:
...
def testStruct(structlist, testsalt, name):
    ...
    for struct in structlist:
        ...
        try:
            ...
            print name, struct, "----   Processing error (missing atom?): ".format(str(e))
            continue
        ...
        #if any(actDists > 30): #activation loop extension
        if actDists[0] > 30 or actDists[1] > 30 or actDists[2] > 28: #activation loop extension
        ...
        print name, struct
        keep.append(struct)
    return keep, chk2, salt, sh2, sdists, hlens

in_keep, in_chk2, in_salt, in_sh2, sdists, in_hlens = testStruct(inlist, True, 'In')
out_keep, out_chk2, dummy, out_sh2, dummy, out_hlens = testStruct(outlist, False, 'Out')
